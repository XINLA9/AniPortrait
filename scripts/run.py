...
def main():
    ...
    for ref_image_path in config["test_cases"].keys():
        ...
        for audio_path in config["test_cases"][ref_image_path]:
            ...
            # 保存 pose 视频（不带音频）
            pose_save_path = f"{save_dir}/{ref_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}_pose.mp4"
            save_videos_grid(
                pose_tensor,
                pose_save_path,
                n_rows=1,
            ...
            # 保存最终生成的视频（暂不加音频）
            result_noaudio_path = f"{save_dir}/{ref_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}_result_noaudio.mp4"
            save_videos_grid(
                video,
                result_noaudio_path,
                n_rows=1,
            ...
            # 合成音频
            stream = ffmpeg.input(result_noaudio_path)
            audio = ffmpeg.input(audio_path)
            final_result_path = result_noaudio_path.replace('_noaudio.mp4', '.mp4')

            ffmpeg.output(
                stream.video, audio.audio,
                final_result_path,
                vcodec='copy', acodec='aac', shortest=None
            ).run()

            # 删除中间无声视频
            os.remove(result_noaudio_path)
